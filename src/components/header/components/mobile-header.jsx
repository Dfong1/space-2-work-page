...
export default function MobileHeader({categories=[], products=[], fullcategories=[]}) {
  const { state } = useCart();
  const {
    dialogOpen,
    sidenavOpen,
  ...
  return (
    <Fragment>
      <FlexBetween width="100%">
        ...
        <Box flex={1}>
          <MobileMenu categories={categories} products={products} fullcategories={fullcategories} />
        </Box>
        ...
        <FlexBox justifyContent="end" flex={1}>
          ...
          {/* <IconButton disabled onClick={toggleDialog}>
            <Icon.User sx={ICON_STYLE} />
          </IconButton>

          <Badge badgeContent={state.cart.length} color="primary">
            <IconButton disabled onClick={toggleSidenav}>
